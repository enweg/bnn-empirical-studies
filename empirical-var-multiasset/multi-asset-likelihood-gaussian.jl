...
function get_yhat_covariance(l, θnet::AbstractVector{T}, θlike::AbstractVector{T}, x::Array{T, 3}) where {T}
    ...
    cov_mat = T.(Float64.(sigmas).*Float64.(correlation_mat).*Float64.(sigmas)') + 1f-3*I
...
function (l::MultiAssetGaussian{T, F, D1, D2})(x::Array{T, 3}, y::Matrix{T}, 
    ...
    # return sum(logpdf(MvNormal(yhat[i, :], cov_mat), y[i, :]) for i=1:size(y, 1)) + sum(logpdf.(tdist_variance, params_variance)) + logpdf(tdist_correlation, Float64.(tcorrelation_mat))
    return sum(logpdf(MvNormal(yhat[:, i], cov_mat), y[:, i]) for i=1:size(y, 1))
end
