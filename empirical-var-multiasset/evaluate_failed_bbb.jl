...
files_vi = readdir("./bbb-objects/")
files_vi = filter(x -> occursin("vi-", x), files_vi)
configs_estimated = [parse(Int, match(r"vi-([0-9]*)", f)[1]) for f in files_vi]

files_evaluated = readdir("./evaluations/")
files_evaluated = filter(x -> occursin("bbb-single-chain-config", x), files_evaluated)
configs_evaluated = [parse(Int, match(r"bbb-single-chain-config([0-9]*)", f)[1]) for f in files_evaluated] 
...
@sync @distributed for i=1:lastindex(configs_todo)
    ...
    try
        vi_data = load(joinpath("bbb-objects", "vi-$config.jld"))
        vi = vi_data["bbb"]
