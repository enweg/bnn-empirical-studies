...
files = readdir("./bbb-objects-simple/")
files = filter(x->occursin("vi-", x), files)
files = [joinpath("bbb-objects-simple", f) for f in files]

logme("Starting evaluation")
@sync @distributed for i=1:lastindex(files)
    # getting chain data
    vi_data = load(files[i])
    netid = vi_data["netid"]
    rep = vi_data["rep"]
    net = network_structures[netid]
    chain = rand(vi_data["bbb"][1], 50_000)
    chain = rand(vi_data["bbb"][1], 20_000)
    ...
    CSV.write("./evaluations/bbb-coeffs-mean-net$netid-rep$rep.csv", coeffs_mean)
    ...
    CSV.write("./evaluations/bbb-qperformance-net$netid-rep$rep.csv", qperformance)
    ...
    CSV.write("evaluations/bbb-stats-net$netid-rep$rep.csv", df)
