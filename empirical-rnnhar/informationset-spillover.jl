...
nreps = 2
netids = repeat(1:lastindex(network_structures), nreps)
reps = repeat(1:nreps; inner = length(network_structures))
...
@sync @distributed for i=1:lastindex(information)
    ...
    try
        ...
        @suppress begin
            ...
            save("chains-spillover/chain-$netid-rep$rep.jld", "netid", netid, "chain", chain, "rep", rep)
        end
...
@sync @distributed for i=1:lastindex(information)
    ...
    try
        ...
        save("bbb-objects-spillover/vi-$netid-rep$rep.jld", "netid", netid, "bbb", vi, "rep", rep)
