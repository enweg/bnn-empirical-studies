...
files = readdir("./bbb-objects-spillover/")
files = filter(x->occursin("vi-", x), files)
files = [joinpath("bbb-objects-spillover", f) for f in files]
...
@sync @distributed for i=1:lastindex(files)
    ...
    CSV.write("./evaluations-spillover/bbb-coeffs-mean-net$netid-rep$rep.csv", coeffs_mean)
    ...
    CSV.write("./evaluations-spillover/bbb-qperformance-net$netid-rep$rep.csv", qperformance)
