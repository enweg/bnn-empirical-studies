...
files = readdir("./chains-simple/")
files = filter(x->occursin("chain-", x), files)
files = [joinpath("chains-simple", f) for f in files]
...
@sync @distributed for i=1:lastindex(files)
    ...
    CSV.write("./evaluations/coeffs-mean-net$netid-rep$rep.csv", coeffs_mean)
    ...
    CSV.write("./evaluations/qperformance-net$netid-rep$rep.csv", qperformance)
    ...
    CSV.write("evaluations/stats-net$netid-rep$rep.csv", df)
