...
@sync @distributed for config_id in 1:lastindex(bbb_configs)
    try
        ...
        netid = findfirst(x -> net == x, network_structures)
        logger("BBB: Starting with config $(config_id)")
        bnn = get_bnn(net, x_train, y_train, nu)
        ...
        save("./bbb-objects/vi-net$config_id.jld", "bbb", vi, "netid", netid)
        logger("BBB: Done with network $(config_id)")
    catch e
        logger("BBB: Error in network $(config_id): $e")
    end
end
