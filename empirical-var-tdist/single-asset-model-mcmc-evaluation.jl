...
@sync @distributed for i in 1:lastindex(mcmc_chains)
    ...
    try
        ...
        CSV.write("./evaluations/mcmc-single-chain-evaluation-config-$chain_config.csv", df)
...
files = files[occursin.("mcmc-single-chain-evaluation-config", files)]
files = ["./evaluations/$f" for f in files]
evaluation = DataFrame(CSV.File(files))
CSV.write("./evaluations/mcmc-single-chain-evaluation-all.csv", evaluation)


# Adding nu information
df = DataFrame(CSV.File("./evaluations/mcmc-single-chain-evaluation-all.csv"))
df[!, "nu"] = [ggmc_configs[config][5] for config in df[!, "config"]]
CSV.write("./evaluations/mcmc-single-chain-evaluation-all.csv", df)
...
@sync @distributed for i in 1:todo
    ...
    try
        ...
        CSV.write("./evaluations/mcmc-same-network-evaluation-chunk$k.csv", df)
...
files = readdir("./evaluations/")
files = files[occursin.("mcmc-same-network-evaluation-chunk", files)]
files = ["./evaluations/$f" for f in files]
evaluation = DataFrame(CSV.File(files))

CSV.write("./evaluations/mcmc-same-network-evaluation-all.csv", evaluation)
