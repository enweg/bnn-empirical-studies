...
benchmark <- read_csv("../tdist-garch11-baseline-multiple.csv")
single_chain <- read_csv("../mcmc-single-chain-evaluation-multiple-all.csv")
same_network <- read_csv("../mcmc-same-network-evaluation-multiple-all.csv")
bbb <- read_csv("../bbb-multiple-asset-all.csv")

benchmark_long <- benchmark %>%
  pivot_longer(cols = -nu, 
               names_to = "measure", 
               values_to = "garch")
...
same_network_evaluation <- same_network %>%
  pivot_longer(-network, 
               names_to = "measure", 
               values_to = "values") %>%
  left_join(benchmark_best, by = "measure") %>%
...
p_single_chain
ggsave("./multiple-asset-single-chain-tgarch.pdf", 
       plot = p_single_chain, 
       device = "pdf")
...
single_chain_comparison %>%
  ...
  summarise(min = min(values), 
            max = max(values), 
            mean = mean(values)) %>%
  write_csv(., file = "./multiple-asset-nu-rmse-tgarch.csv")
...
p_same_network <- same_network_evaluation %>%
  ...
  geom_hline(aes(yintercept = garch, color = "Garch(1, 1)"), size = 0.5) + 
  geom_hline(aes(yintercept = target, color = "Target"), size = 1, linetype = "dotted") + 
...
p_same_network
ggsave("./multiple-asset-same-network-tgarch.pdf", 
       plot = p_same_network, 
       device = "pdf")
...
ggsave("./multiple-asset-mixing-performance.pdf", 
