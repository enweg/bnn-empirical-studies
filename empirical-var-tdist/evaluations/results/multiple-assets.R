...
benchmark <- read_csv("../garch11-baseline-multiple.csv")
...
p_single_chain
ggsave("./multiple-asset-single-chain.pdf", 
       plot = p_single_chain, 
       device = "pdf")
...
single_chain_comparison %>%
  ...
  summarise(min = min(values), 
            max = max(values), 
            mean = mean(values)) %>%
  write_csv(., file = "./multiple-asset-nu-rmse.csv")
...
p_same_network
ggsave("./multiple-asset-same-network.pdf", 
       plot = p_same_network, 
       device = "pdf")
